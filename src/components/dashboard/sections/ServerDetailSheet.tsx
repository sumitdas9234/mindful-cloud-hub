      ...
      <SheetContent className="w-[400px] sm:w-[540px] overflow-y-auto">
        <SheetHeader className="pb-4">
          <SheetTitle className="text-2xl flex items-center gap-2">
            ...
            <Badge 
              variant="outline" 
              className={`ml-2 capitalize ${server.status === 'online' ? 'text-green-500 border-green-500' : 
                server.status === 'offline' ? 'text-red-500 border-red-500' : 
                'text-amber-500 border-amber-500'}`}
            >
              <span className={`mr-1.5 h-2 w-2 rounded-full inline-block ${getStatusColor(server.status)}`}></span>
              {server.status}
            </Badge>
          </SheetTitle>
          <SheetDescription>
            ...
            <CardContent>
              <dl className="space-y-2">
                ...
                <div className="flex justify-between">
                  ...
                  <dd>{server.name.includes('app') ? 'Application Server' : 
                       server.name.includes('db') ? 'Database Server' : 
                       server.name.includes('cache') ? 'Cache Server' : 'General Purpose'}</dd>
                </div>
                <Separator />
                <div className="flex justify-between">
                  <dt className="text-muted-foreground">Environment:</dt>
                  <dd>{server.name.includes('prod') ? 'Production' : 
                       server.name.includes('dev') ? 'Development' : 
                       server.name.includes('test') ? 'Testing' : 
                       server.name.includes('staging') ? 'Staging' : 
                       server.name.includes('backup') ? 'Backup' : 'Unknown'}</dd>
                </div>
              </dl>
            </CardContent>
          </Card>
